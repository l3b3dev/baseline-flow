...
def main(argv=None):
    ...
    sens_data_name300 = "results/sens_sensitivity_error_s"+str(n_sensors)+ \
                        "e100_samp"+str(n_samp) + "l800_pert"+ str(pert) + "_Re300.npz"
    ...
    morris_mean_abs_total155 = np.sqrt(np.sum(morris_mean_abs155**2, axis = 1))
    morris_mean_abs_total300 = np.sqrt(np.sum(morris_mean_abs300**2, axis = 1))
    
    
    morris_std_total155 = np.sqrt(np.sum(morris_std155**2, axis = 1))
    morris_std_total300 = np.sqrt(np.sum(morris_std300**2, axis = 1))
    
    
    #Plot total sensitivities subplots
    plot_ReSubplot(savefolder + "total_mean_abs",
                   morris_mean_abs_total155, morris_mean_abs_total300,
                   sensor_locations_2D155, sensor_locations_2D300,
                   flow1, flow2, "Re=15500", "Re=30000", 
                   r"Total  $\mu^*$ for Reconstruction Error", figsize = (7.2,3.5))
    
    
    plot_ReSubplot(savefolder + "total_std",
                   morris_std_total155, morris_std_total300,
                   sensor_locations_2D155, sensor_locations_2D300,
                   flow1, flow2, "Re=15500", "Re=30000", 
                   r"Total  $\sigma$ for Reconstruction Error", figsize = (7.2,3.5))
    
    #Plot total sensitivity singleplots
    plot_ReSingle(savefolder + "total_mean_abs_Re155",
                  morris_mean_abs_total155,sensor_locations_2D155, flow1,
                  "Re=15500", r"Total $\mu^*$",s=s)
    
    plot_ReSingle(savefolder + "total_mean_abs_Re300",
                  morris_mean_abs_total300,sensor_locations_2D300, flow2,
                  "Re=30000", r"Total $\mu^*$",s=s)
    
    plot_ReSingle(savefolder + "total_std_Re155",
                  morris_std_total155,sensor_locations_2D155, flow1,
                  "Re=15500", r"Total $\sigma$",s=s)
    
    plot_ReSingle(savefolder + "total_std_Re300",
                  morris_std_total300,sensor_locations_2D300, flow2,
                  "Re=30000", r"Total $\sigma$", s=s)
